...
function onePageScroll(element, options) {
  ...
  this.init = function() { 
    ...
    /*-------------------------------------------*/
      ...
      var pagination_links_button = document.querySelectorAll(".scroll_button");
    
  	  for( var i = 0; i < pagination_links.length; i++){
    	  pagination_links[i].addEventListener('click', _paginationHandler);
      }

      for( var i = 0; i < pagination_links_button.length; i++){
        pagination_links_button[i].addEventListener('click', _paginationHandler);
      }
  	}
